    ...
    if st.button("기사 제목 만들기", key="generate-similar-titles-button"):
        ...
        with st.spinner('GPT-4가 기사 제목을 만들고 있어'):
            if selected_titles:  # 선택된 기사 제목이 있다면
                prompt = "너는 기사 제목을 잘 뽑는 한국의 편집기자야. selected_titles의 기사 제목들 중에서 3개에서 5개를 알아서 뽑아 줘. 그리고 뽑은 제목에서 조사나 부사만 살짝 바꿔서 보여줘. 고유명사는 절대 바꾸지 말고, 원래 제목에 없는 단어는 절대 추가하지 마. 만약 與 한자가 나오면 여로 바꾸고, 野가 나오면 야로 바꿔. 李가 나오면 이, 尹이 나오면 윤으로 바꿔. 이렇게 딱 한 자의 한자가 나오면 그 글자만 한그로 바꿔. 원래 한글인 것은 절대로 한자로 풀어서 쓰지 마. 단어의 나열 순서 등을 원래대로 유지해. 원래 제목에서 " " 이렇게 큰 따옴표와 ' ' 이렇게 작은 따옴표에 들어있는 내용들은 똑같게 유지해. () [] 이런 괄호들에 들어있는 내용은 무조건 빼. 새롭게 만들어 준 기사 제목만 보여줘."

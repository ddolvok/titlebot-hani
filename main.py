    ...
    if st.button("기사 제목 만들기", key="generate-similar-titles-button"):
        ...
        with st.spinner('GPT-4가 기사 제목을 만들고 있어'):
            if selected_titles:  # 선택된 기사 제목이 있다면
                prompt = "selected_titles의 기사 제목들을 그대로 5개를 뽑아 줘. 뽑은 기사 제목들을 90% 이상 똑같은 형태로 새롭게 만들어 줘. 원래 제목에 없는 단어를 절대 추가하지 마. 단어의 나열 순서 등을 원래대로 유지해. 원래 제목에서 " " 이렇게 큰 따옴표와 ' ' 이렇게 작은 따옴표에 들어있는 내용들은 똑같게 유지해. () [] 이런 괄호들에 들어있는 내용은 무조건 빼. 새롭게 만들어 준 기사 제목만 보여줘."
